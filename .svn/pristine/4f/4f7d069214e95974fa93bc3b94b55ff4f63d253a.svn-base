	...
	IF old.trigger_flag = new.trigger_flag AND new.service_code <> '' AND new.customer_code <> '' THEN
		UPDATE event_items ei
       	...
       	SET ei.service_code = n.service_code,
       		...
       		ei.is_modified = (CASE WHEN n.status=2 THEN '1' ELSE '0' END),
       		ei.status = (CASE WHEN n.status=7 THEN '-12' WHEN n.status=0 THEN '-11' WHEN n.status=6 THEN '-2' WHEN n.status=5 THEN '2'  WHEN n.status=1 OR n.status=2 THEN '1' ELSE '0' END);
