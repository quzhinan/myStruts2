...
CREATE TRIGGER `trigger_phase1to2_insert` BEFORE INSERT ON nursing FOR EACH ROW
BEGIN
	...
	IF new.trigger_flag = 0 AND new.service_code <> '' AND new.customer_code <> '' THEN
		...
		INSERT INTO event_items 
			...
			comment_content, comment_update_datetime, comment_confirm_status)
		VALUES 
			(new.service_code, REPLACE(new.visit_date, '-', ''), REPLACE(new.from_time, ':', ''), REPLACE(new.end_time, ':', ''),
			 ...
			 new.comment_content, new.comment_update_datetime, new.comment_confirm_status
			);
